...
public class CilentActivity extends Activity {
	private TextView cilent_tv1;
	private Button cilent_bt1;
	private Button cilent_bt2;
	/****************************************
	 * 扫雷相关变量
	 */
	// 创建相同地图
	private int mineRow, mineColumn;
	private int ThemineRow[] = new int[41];
	private int ThemineColumn[] = new int[41];
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.cilentactivity);
		cilent_tv1 = (TextView) findViewById(R.id.cilent_tv1);
		cilent_bt1 = (Button) findViewById(R.id.cilent_bt1);
		cilent_bt2 = (Button) findViewById(R.id.cilent_bt2);
		cilent_bt1.getBackground().setColorFilter(0xFFEEEE00, PorterDuff.Mode.MULTIPLY);
		cilent_bt2.getBackground().setColorFilter(0xFFEEEE00, PorterDuff.Mode.MULTIPLY);
		...
		cilent_bt1.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				Intent serverIntent = new Intent(CilentActivity.this,
						DeviceListActivity.class);
				startActivityForResult(serverIntent, REQUEST_CONNECT_DEVICE);
			}
		});
		cilent_bt2.setOnClickListener(new OnClickListener() {
	...
	private void sendMessage(String message) {
		...
		if (mChatService.getState() != BluetoothChatService.STATE_CONNECTED) {
			Toast.makeText(this, R.string.no_connect, Toast.LENGTH_SHORT)
					.show();
			return;
		}
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_STATE_CHANGE:
				...
				case BluetoothChatService.STATE_CONNECTED:
					cilent_tv1.setText(R.string.connected);
					cilent_tv1.append(mConnectedDeviceName);

					break;
				case BluetoothChatService.STATE_CONNECTING:
					cilent_tv1.setText(R.string.connecting);
					break;
				case BluetoothChatService.STATE_LISTEN:
				case BluetoothChatService.STATE_NONE:
					cilent_tv1.setText(R.string.no_connect);
					break;
				}
				break;
			case MESSAGE_WRITE:
			...
			case MESSAGE_READ:
				...
				if (k != -1) {
					...
					{
						int k1 = readMessage.indexOf(",", j);
						mineRow = Integer
								.parseInt(readMessage.substring(j, k1));
						ThemineRow[i] = mineRow;
						Log.v("!!!!!!!!!!!", ThemineRow[i] + "");
						int k2 = readMessage.indexOf(",", k1 + 1);
						mineColumn = Integer.parseInt(readMessage.substring(
								k1 + 1, k2));
						ThemineColumn[i] = mineColumn;
						Log.v("************", ThemineColumn[i] + "");
						j = k2 + 1;
				...
				if (l != -1) {
					...
					int y2 = Integer.parseInt(readMessage
							.substring(l1 + 1, len));
					rippleUncover(y1, y2);
					...
					showDialog("现在轮到你扫雷......", 500, true, false);
				}
				if (p != -1) {
					...
					findCount2++;
					if (findCount2 < 10) {
						txtTimer.setText("0" + findCount2);
					}
					if (findCount2 >= 10) {
						txtTimer.setText("" + findCount2);
					}
					...
					public void onClick(View view) {
						...
						if (!blocks[currentRow][currentColumn].isFlagged()) {
							...
							if (blocks[currentRow][currentColumn].hasMine()) {
								...
								findCount++;
								if (findCount < 10) {
									txtTimer2.setText("0" + findCount);
								}
								if (findCount >= 10) {
									txtTimer2.setText("" + findCount);
								}
								updateMineCountDisplay();
							...
							sendMessage(Uncover);
							if (toast) {
								showDialog("等待对方扫雷......", 500, true, false);
							}
	...
	private void winGame() {
		...
		showDialog("YOU WIN", 1000, true, false);
	...
	private void loseGame() {
		...
		showDialog("YOU LOSE", 1000, false, false);
	}

	private void setMines() {
		for (int row = 0, i = 0; row < totalNumberOfMines; row++, i++) {

			mineRow = ThemineRow[i];
			Log.v(";;;;;;;;;;;;;;;;;;;;", ThemineRow[i] + "");
			mineColumn = ThemineColumn[i];
			Log.v(";;;;;;;;;;;;;;;;;;;;++", ThemineColumn[i] + "");
